...
class ResponsiveState implements ResponsiveStateProps {
    private matcher;
...
let categories: {
    [id: string]: ResponsiveCategory
};

Object.keys(states).filter(key => key).forEach(category => {
    categories[category] = new ResponsiveCategory(category);
    Object.keys(states[category]).filter(key => key).forEach(state => categories[category].addState(new ResponsiveState(state, states[category][state])));
});